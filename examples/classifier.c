...
void predict_classifier2(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top, char *test_filepath, char *out_file)
{
    FILE *fp;
    ...
    char *input = buff;
	////////////// Zahid ////////////////
    list *plist = get_paths(test_filepath);
    char **paths = (char **)list_to_array(plist);
    int m = plist->size;
    free_list(plist);
    	...
    	////////////////// Zahid //////////////
    for(j = 0; j < m; ++j){
        image im = load_image_color(paths[j], 0, 0);
    /*while(1){ // Zahid
        if(filename){
            strncpy(input, filename, 256);
        }else{
...
void run_classifier(int argc, char **argv)
{
    ...
    char *test_filepath = find_char_arg(argc, argv, "-test_filename", 0);
    char *out_file = find_char_arg(argc, argv, "-out_filename", 0);
    char *data = argv[3];
    char *cfg = argv[4];
    ...
    if(0==strcmp(argv[2], "predict2")) predict_classifier2(data, cfg, weights, filename, top, test_filepath, out_file);
    else if(0==strcmp(argv[2], "predict")) predict_classifier(data, cfg, weights, filename, top);
