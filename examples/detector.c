...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        if(i%10000==0 || (i < 1000 && i%100 == 0) || (i%1000 == 0)){
#ifdef GPU
            if(ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            char buff[256];
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
	...
	else if(fgets(input, 130, fp) != NULL){
//	    input = getline(&input, &len, fp)) != -1;
		char *sub=buff;
		memcpy(sub, &input[0], strlen(input)-2);
		sub[strlen(input)-1] = '\0';
		//printf("Input length: %d\n", strlen(input));
		
	     //printf("Image Path sub: %s\n", sub);
	     printf("Enter Image Path: %s\n", input);
	}
	else {
        ...
        float *X = sized.data;
        time=what_time_is_it_now();//zahid
        network_predict(net, X);
        //network_predict_zahid(net, X, input);
	...
	char *out_filename = buff;
	char *out_dir; out_dir = "out/";
	while (token != NULL) {
		out_filename = token;
		token = strtok(NULL, "/");
	}
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        draw_detections(im, dets, nboxes, thresh, names, alphabet, l.classes, out_filename);
        free_detections(dets, nboxes);
        if(outfile){
            save_image(im, outfile);
        }
	else if(multi){
	    char* str3 = (char *) malloc(1 + strlen(out_dir)+ strlen(out_filename) );
		strcpy(str3, out_dir);
	      strcat(str3, out_filename);
	      printf("%s", str3);
//	    save_image(im, out_filename);
	    save_image(im, str3);
		free(str3);
	}
        else{
